...
argument-hint: "次セッションで何をするか (省略可。'brief' で短縮版)"
---
...
2. **次のテンプレートで Markdown を組み立てる**:
   ...
   ## 次セッションで使うと良い skill / コマンド
   - <例: `/wiki-query`, `/save`, `/review`, `/security-review` など、次セッションで効きそうなもの。無ければ省略>
...
- `/handoff` — フル形式 (5 セクション全部)
- `/handoff brief` — 「直近セッションでやったこと」+ 「次に着手するなら」 のみの短縮版
- `/handoff <次セッションの目的>` — フリー記述で次セッションのフォーカスを渡す。その目的に関係ない項目は思い切って省く (例: `/handoff PR #42 のレビュー対応`)

## ガイドライン

- 推測で書かない。conversation で明確に話題に出ていない事項を勝手に「次に着手」に入れない
- **重複させない**: PR / plan / commit message / wiki ページ / issue にすでに書かれている内容は本文にコピペせず、パス・URL・PR 番号で参照する。handoff は索引であって全文コピーではない
